...
export const WardenComplaintContextProvider = ({ children }) => {
    ...
    const FetcheAllUsers = async () => {
        try {
            const response = await fetch(
                "http://localhost:8000/api/v1/complaints/get-users",
                {
                    method: "GET",
                    headers: {
                        Authorization: Token,
            ...
            if (response.ok) {
                ...
                setsortedUsersList(responseData.data);
            } else {
    ...
    const getComplaintsByType = async (Type) => {
        ...
        try {
            const response = await fetch(
                `http://localhost:8000/api/v1/complaints/${Type}/get-complaints-by-type`,
                {
                    method: "GET",
                    headers: {
                        Authorization: Token,
            ...
            console.log(
                "waredencomplaintcontext:: getcomplaintsByType ::",
                error.message
            );
        }
    };
    ...
    const filterFetchedComplaints = (state) => {
        const complaints = FetchedComplaint.filter(
    ...
    const getComplaintDetail = (complaintId) => {
        const FoundComplaint = ComplaintToBeDisplayed?.find(
    ...
    const saveTheUpdatedState = async (complaintObj) => {
        try {
            const response = await fetch(
                `http://localhost:8000/api/v1/complaints/${complaintObj.complaintId}/edit-complaint-state`,
                {
                    method: "PATCH",
                    headers: {
                        Authorization: Token,
    ...
    const addMealPoll = async (mealPollData) => {
        try {
            const response = await fetch(
                "http://localhost:8000/api/v1/meals/add-meal",
                {
                    method: "POST",
                    headers: {
                        Authorization: Token,
            ...
            if (response.ok) {
                localStorage.setItem(
                    "MealPollId",
                    JSON.stringify(responseData.data._id)
                );
    ...
    const getMealPollById = async (pollId) => {
        try {
            const response = await fetch(
                `http://localhost:8000/api/v1/meals/${pollId}/get-mealPoll-by-id`,
                {
                    method: "GET",
                    headers: {
                        Authorization: Token,
    ...
    const saveTheUpdatedMealStatus = async (MealObj) => {
        try {
            const response = await fetch(
                `http://localhost:8000/api/v1/meals/${MealObj.mealId}/update-state`,
                {
                    method: "PATCH",
                    headers: {
                        Authorization: Token,
            ...
            toast.error(responseData.message);
            console.log(
                "wardencontext :: saveTheUpdatedMealStatus ::",
                error.message
            );
        }
    };
    ...
    const getLatestNoticePosted = async () => {
        try {
            const response = await fetch(
                "http://localhost:8000/api/v1/notice/get-latest-notice",
                {
                    method: "GET",
                    headers: {
                        Authorization: Token,
    ...
    // Note: below functions are for AllStudentdeatis page

    const getFilteredUserList = (filterCriteria) => {
        try {
            const { department, studentEmailOrPhone } = filterCriteria;
            ...
            if (department && studentEmailOrPhone) {
                filteredList = UsersList.filter(
                    (user) =>
                        ...
                        (user.email === studentEmailOrPhone || user.phone === studentEmailOrPhone)
                );
            } else if (department) {
                filteredList = UsersList.filter((user) => user.branchName === department);
            } else if (studentEmailOrPhone) {
                filteredList = UsersList.filter(
                    (user) => user.email === studentEmailOrPhone || user.phone === studentEmailOrPhone
                );
            } else {
                filteredList = UsersList;
            }

            setsortedUsersList(filteredList);

        } catch (error) {
            toast.error("Error filtering users based on the provided criteria.");
        }
    };

    //Note: below functions are for single student complaint
    ...
    return (
        <complaintcontext.Provider
            value={{
                ...
                // Note:this is for all users list page
                getFilteredUserList,
                setsortedUsersList

                // Note: this is for single user complaints
